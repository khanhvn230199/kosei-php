// JavaScript Document
function logout() {
  if (confirm("Bạn có muốn thoát khỏi hệ thống?\n\n[OK]: Đồng ý     [Cancel]: Hủy")) {
    window.location.href = "?mod=_login&act=logout";
    return true;
  }
  return false;
}
...
function SelectAll(frmName) {
  ...
  if (f.elements["chkItem"][0]) {
    for (var i = 0; i < f.elements["chkItem"].length; i++) f.elements["chkItem"][i].checked = frmName.checked;
  } else {
