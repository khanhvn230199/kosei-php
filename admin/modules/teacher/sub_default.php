...
function default_default()
{
    ...
    makeArrayListCategory(0, 0, $_max_category_level, $arrOptionsCategory, "ctype=" . CTYPE_GV);
    makeArrayListAuthor($arrAuthor, 1);
    //init Grid
    $baseUrl = "?mod=$mod";
    $cond = "lang_code='$lang_code' AND ctype=" . CTYPE_GV;
    if ($_GET["return"] != "") {
    ...
    $clsDataGrid->setTitle($core->getLang("Teacher"));
...
function default_add()
{
    ...
    $arrTemplateOption[CTYPE_GV]['default'] = "Mặc định (kế thừa từ nhóm)";
    //init Form
    $clsForm = new Form();
    if ($mode == "New") {
    ...
    $clsForm->setTitle($core->getLang("Teacher"));
    ...
    $clsForm->addInputText("facebook", "", "Facebook", 255, 1, "style='width:99%'");
    $clsForm->addInputText("instagram", "", "Instagram", 255, 1, "style='width:99%'");
    $clsForm->addInputText("twitter", "", "Twitter", 255, 1, "style='width:99%'");
    ...
    if ($btnSave != "") {
        $action_log = "";
        if ($mode == "New") {
            $clsForm->addInputHidden('ctype', CTYPE_GV);
