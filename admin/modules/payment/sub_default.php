...
function default_default()
{
    ...
    global $core, $clsModule, $clsButtonNav, $arrYesNoOptions, $paymentMethodOptions;
    global $lang_code;
    $classTable = "Payment";
    ...
    //################### CHANGE BELOW CODE ###################
    $htmlOptionsCtype = makeHTMLOptions($paymentMethodOptions, $ctype);
    //init Grid
    $baseUrl = "?mod=$mod";
    $cond = "ctype=$ctype AND lang_code='$lang_code'";
    if ($_GET["return"] != "") {
    ...
    $clsDataGrid->setTitle($core->getLang("Payment"));
    ...
    if ($ctype == 0) {
        $clsDataGrid->addColumnLabel("account_holder", "Chủ tài khoản", "width='15%'");
        $clsDataGrid->addColumnLabel("account_number", "STK ngân hàng", "width='15%'");
        $clsDataGrid->addColumnSelect("country", "Quốc gia", "width='2%' align='center'", array('vn' => 'Việt Nam', 'jp' => 'Nhật bản'));
    }
    $clsDataGrid->addColumnSelect("ctype", "Phương thức thanh toán", "width='15%'", $paymentMethodOptions, 0, 1);
    ...
    $assign_list["htmlOptionsCtype"] = $htmlOptionsCtype;
...
function default_add()
{
    ...
    $clsForm->setTitle($core->getLang("Payment"));
    $clsForm->setTextAreaType("full");
    $clsForm->addInputHidden("ctype", $ctype);
    if ($ctype == 0) {
        $clsForm->addInputText("name", "", "Tên ngân hàng", 255, 0, "style='width:99%'");
        $clsForm->addInputText("account_holder", "", "Chủ tài khoản", 255, 0, "style='width:99%'");
        $clsForm->addInputText("account_number", "", "STK ngân hàng", 255, 0, "style='width:99%'");
        $clsForm->addInputSelect('country', "", "Quốc gia", array('vn' => 'Việt Nam', 'jp' => 'Nhật bản'));
    } else {
...
function default_delete()
{
    ...
    $tableName = $clsClassTable->tbl;
    $pkeyTable = $clsClassTable->pkey;
    $_arr_Payment_template = $core->getLangArray($_arr_Payment_template);
    $return = (isset($_GET["return"])) ? base64_decode($_GET["return"]) : "";
    if ($return == "") $return = "mod=$mod";
    ...
    if ($pvalTable != "") {
        ...
        $clsRecycleBin->AddNew($classTable, $pvalTable, "name", "Payment");
        //End RecycleBin
    ...
    if (is_array($checkList)) {
        foreach ($checkList as $key => $val) {
            //Begin RecycleBin
            $clsRecycleBin = new RecycleBin();
            $clsRecycleBin->AddNew($classTable, $val, "name", "Payment");
            //End RecycleBin
