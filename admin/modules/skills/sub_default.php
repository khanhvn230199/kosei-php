...
function default_default()
{
    ...
    $clsDataGrid->setTitle($core->getLang("Skills"));
...
function default_add()
{
    ...
    $clsForm->setTitle($core->getLang("Skills"));
    $clsForm->setTextAreaType("full");
    $clsForm->addInputText("name", "", "SkillName", 255, 0, "style='width:99%'");
...
function default_delete()
{
    ...
    $tableName = $clsClassTable->tbl;
    $pkeyTable = $clsClassTable->pkey;
    $_arr_Skills_template = $core->getLangArray($_arr_Skills_template);
    $return = (isset($_GET["return"])) ? base64_decode($_GET["return"]) : "";
    if ($return == "") $return = "mod=$mod";
    ...
    if ($pvalTable != "") {
        ...
        $clsRecycleBin->AddNew($classTable, $pvalTable, "name", "Skills");
        //End RecycleBin
    ...
    if (is_array($checkList)) {
        foreach ($checkList as $key => $val) {
            //Begin RecycleBin
            $clsRecycleBin = new RecycleBin();
            $clsRecycleBin->AddNew($classTable, $val, "name", "Skills");
            //End RecycleBin
