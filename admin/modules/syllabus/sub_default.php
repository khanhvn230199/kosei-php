...
function default_default()
{
    ...
    makeArrayListCategory(0, 0, $_max_category_level, $arrOptionsCategory, "ctype=" . CTYPE_GT);
    makeArrayListAuthor($arrAuthor, 1);
    //init Grid
    $baseUrl = "?mod=$mod";
    $cond = "lang_code='$lang_code' AND ctype=" . CTYPE_GT;
    if ($_GET["return"] != "") {
    ...
    $clsDataGrid->setTitle($core->getLang("Syllabus"));
...
function default_add()
{
    ...
    $arrTemplateOption[CTYPE_GT]['default'] = "Mặc định (kế thừa từ nhóm)";
    //init Form
    $clsForm = new Form();
    if ($mode == "New") {
    ...
    $clsForm->setTitle($core->getLang("Syllabus"));
    ...
    if ($btnSave != "") {
        $action_log = "";
        if ($mode == "New") {
            $clsForm->addInputHidden('ctype', CTYPE_GT);
