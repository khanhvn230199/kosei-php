...
function default_default(){
	...
	//End Init
	//Begin SQl Condition
	$cond = "a.lang_code='$_LANG_ID' AND a.is_online=1";	
	if ($cat_id!="" && $cat_id!="0"){
	...
	$cond2 = "a.lang_code='$_LANG_ID' AND a.is_online=1 AND a.user_id>1";
	//End SQL Condition
	//Begin Paging
	$rowPerPage = 12;
	$curPage = (isset($_GET["page"]) && $_GET["page"]>0)? ($_GET["page"]) : 0;
	$clsPaging = new Paging($curPage, $rowPerPage, "syllabus");
	...
	//End Paging
	//Begin ListArticles
	$arrListSyllabus = $clsArticles->getAllSimple($clsPaging->getQueryLimit($cond.$orderby));
	if ($curCat['parent_id']==0){
		$arrListSyllabusByCat = $clsCategory->getSubCatNews($cat_id);
	}else{
		$arrListSyllabusByCat = $clsCategory->getSubCatNews($curCat['parent_id'], 0);
	...
	$assign_list["arrListSyllabus"] = $arrListSyllabus;
	$assign_list["arrListSyllabusByCat"] = $arrListSyllabusByCat;
	...
	if ($page_title=="") $site_title = "Giáo trình";
