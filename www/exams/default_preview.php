...
/**
 * Module: [Exams]
 * Posts function with $sub=default, $act=preview
 * Display preview of a exam
 *
 * @param                : no params
...
function default_preview()
{
    ...
    if (is_array($arrListQuestions) && count($arrListQuestions) > 0) {
        foreach ($arrListQuestions as $q => $question) {
            $arrChild = $clsQuestions->getAll("exam_id = $exam_id AND parent_id = $question[questions_id] AND lang_code = '$_LANG_ID' AND is_online = 1 ORDER BY rand() LIMIT $question[question_limit]");
